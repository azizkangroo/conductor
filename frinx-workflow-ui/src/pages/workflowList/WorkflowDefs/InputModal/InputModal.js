...
function InputModal(props) {
  ...
  const getWaitingWorkflows = () => {
    return new Promise((resolve, reject) => {
      ...
      http.get(global.backendApiUrlPrefix + '/workflow/search?q=&h=&freeText=' + q + '&start=' + 0 + '&size=').then(res => {
        const runningWfs = res.results?.totalHits || [];
        const promises = runningWfs.map(wf => {
          return http.get(global.backendApiUrlPrefix + '/workflow/' + wf.workflowId);
        });
