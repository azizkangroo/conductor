...
class DiagramBuilder extends Component {
  ...
  componentDidMount() {
    ...
    http.get(this.context.backendApiUrlPrefix + '/metadata/workflow').then(res => {
      this.props.storeWorkflows(res?.sort((a, b) => a.name.localeCompare(b.name)) || []);
    });

    http.get(this.context.backendApiUrlPrefix + '/metadata/taskdefs').then(res => {
      this.props.storeTasks(res?.sort((a, b) => a.name.localeCompare(b.name)) || []);
    });
  ...
  createExistingWorkflow() {
    ...
    http
      .get(this.context.backendApiUrlPrefix + '/metadata/workflow/' + name + '?version=' + version)
      .then(res => {
        this.createDiagramByDefinition(res);
      })
      .catch(() => {
  ...
  closeInputModal(openDetails) {
    this.setState({
      showInputModal: false,
      showDetailsModal: openDetails,
    });
  }
