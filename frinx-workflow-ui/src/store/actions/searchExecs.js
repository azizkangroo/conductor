...
export const fetchNewData = (viewedPage, defaultPages, backendApiUrlPrefix) => {
  return (dispatch, getState) => {
    ...
    http
      .get(backendApiUrlPrefix + '/workflow/search?q=&h=&freeText=' + q + '&start=' + page + '&size=' + defaultPages)
      .then(res => {
        if (res.results) {
          const data = res.results ? (res.results ? res.results : []) : [];
          dispatch(updateSize(res.totalHits));
...
export const fetchParentWorkflows = (viewedPage, defaultPages, backendApiUrlPrefix) => {
  return (dispatch, getState) => {
    ...
    http
      .get(
        backendApiUrlPrefix + '/workflow/search?freeText=' + q + '&start=' + checkedWfs[page] + '&size=' + defaultPages,
      )
      .then(res => {
        if (res) {
          let parents = res.parents ? res.parents : [];
          let children = res.children ? res.children : [];
          if (res.count < res.totalHits && (typeof checkedWfs[viewedPage] === 'undefined' || checkedWfs.length === 1)) {
