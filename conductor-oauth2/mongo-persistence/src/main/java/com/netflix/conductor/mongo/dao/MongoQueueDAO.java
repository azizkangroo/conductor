...
public class MongoQueueDAO extends MongoBaseDAO implements QueueDAO {
    ...
    public boolean setUnackTimeout(String queueName, String messageId, long unackTimeout) {
        ...
        return mongoTemplate
                        .updateMulti(query, update, QueueMessageDocument.class)
                        .getModifiedCount()
                == 1;
    }
    ...
    public Map<String, Long> queuesDetail() {
        ...
        List<String> queueNames =
                mongoTemplate.findDistinct(
                        findQuery, "queue_name", QueueMessageDocument.class, String.class);
    ...
    public Map<String, Map<String, Map<String, Long>>> queuesDetailVerbose() {
        ...
        List<String> queueNames =
                mongoTemplate.findDistinct(
                        new Query(), "queue_name", QueueMessageDocument.class, String.class);
        ...
        queueNames.forEach(
                queueName -> {
                    ...
                    unackedQuery.addCriteria(
                            Criteria.where("popped").is(true).and("queue_name").is(queueName));
                    long queueUnacked =
                            mongoTemplate.count(unackedQuery, QueueMessageDocument.class);

                    result.put(
                            queueName,
                            ImmutableMap.of(
                                    "a",
                                    ImmutableMap.of( // sharding not implemented, returning only
    ...
    public boolean resetOffsetTime(String queueName, String messageId) {
        QueueMessageDocument aQueueMessageDocument =
                mongoTemplate.findOne(
                        new Query()
                                .addCriteria(
                                        Criteria.where("queue_name")
                                                .is(queueName)
                                                .and("message_id")
                                                .is(messageId)),
                        QueueMessageDocument.class);
        if (null == aQueueMessageDocument) return false;
        else {
    ...
    private void pushMessage(
        ...
        if (null != aQueueMessageDocument) {
            ...
            aQueueMessageDocument.setDeliverOn(
                    getOffsetAddedDate(((Long) offsetTimeInSecond).intValue()));
            ...
            newQueueMessageDocument.setDeliverOn(
                    getOffsetAddedDate(((Long) offsetTimeInSecond).intValue()));
    ...
    private List<Message> peekMessages(String queueName, int count) {
        ...
        if (!aQueueMessageDocumentList.isEmpty())
            aQueueMessageDocumentList.forEach(
                    qmd -> {
    ...
    private boolean removeMessage(String queueName, String messageId) {
        QueueMessageDocument queueMessageDocument =
                mongoTemplate.findOne(
                        new Query()
                                .addCriteria(
                                        Criteria.where("queue_name")
