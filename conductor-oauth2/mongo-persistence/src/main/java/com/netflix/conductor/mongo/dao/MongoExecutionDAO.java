...
public class MongoExecutionDAO extends MongoBaseDAO
        implements ExecutionDAO, RateLimitingDAO, PollDataDAO {
    ...
    public List<Task> getPendingTasksByWorkflow(String taskName, String workflowId) {
        ...
        if (!inProgress.isEmpty()) {
            ...
            mongoTemplate
                    .find(orQuery, TaskDataDocument.class)
                    .forEach(
                            tdd -> {
        ...
        for (Task task : tasks) {
            try {
                ...
                throw new ApplicationException(
                        ApplicationException.Code.NOT_FOUND, npe.getMessage());
            }
    ...
    public Task getTask(String taskId) {
        ...
        return null != taskDocument && null != taskDocument.getJson_data()
                ? readValue(
            ...
            if (!taskIds.isEmpty()) {
                ...
                orQuery.addCriteria(
                        orCriteria.orOperator(
                                orExpression.toArray(new Criteria[orExpression.size()])));
                List<TaskDataDocument> taskDataList =
                        mongoTemplate.find(orQuery, TaskDataDocument.class);

                if (taskDataList.isEmpty()) return result;
                else
    ...
    public List<Task> getTasksForWorkflow(String workflowId) {
        ...
        mongoTemplate
                .find(searchQuery, WorkflowToTaskDocument.class)
    ...
    public List<String> getRunningWorkflowIds(String workflowName, int version) {
        ...
        mongoTemplate
                .find(searchQuery, WorkflowPendingDocument.class)
    ...
    public List<Workflow> getWorkflowsByType(String workflowName, Long startTime, Long endTime) {
        ...
        List<String> workflowIds = new ArrayList<String>();
        mongoTemplate
                .find(searchQuery, WorkflowDefToWorkflowDocument.class)
    ...
    public List<Workflow> getWorkflowsByCorrelationId(
        ...
        if (!inProgress.isEmpty()) {
            ...
            for (WorkflowDefToWorkflowDocument aWorkflowDefToWorkflowDocument : inProgress) {
                Criteria expression =
                        Criteria.where("workflow_id")
                                .is(aWorkflowDefToWorkflowDocument.getWorkflow_id())
                                .and("correlation_id")
                                .is(correlationId);
                orExpression.add(expression);
            }
            orQuery.addCriteria(
                    orCriteria.orOperator(orExpression.toArray(new Criteria[orExpression.size()])));
            mongoTemplate
                    .find(orQuery, WorkflowDocument.class)
                    .forEach(
                            wd -> {
    ...
    public void updateEventExecution(EventExecution eventExecution) {
        ...
        updateQuery.addCriteria(
                Criteria.where("event_handler_name")
                        .is(eventExecution.getName())
                        .and("event_name")
                        .is(eventExecution.getEvent())
                        .and("message_id")
                        .is(eventExecution.getMessageId())
                        .and("execution_id")
                        .is(eventExecution.getId()));
    ...
    boolean addScheduledTask(Task task, String taskKey) {
        ...
        checkExistsQuery.addCriteria(
                Criteria.where("workflow_id")
                        .is(task.getWorkflowInstanceId())
                        .and("task_key")
                        .is(taskKey));
    ...
    private void addTaskInProgress(Task task) {
        ...
        Query searchQuery = new Query();
        searchQuery.addCriteria(
                Criteria.where("task_def_name")
                        .is(task.getTaskDefName())
                        .and("task_id")
                        .is(task.getTaskId()));
    ...
    private List<String> findAllTasksInProgressInOrderOfArrival(Task task, int limit) {
        ...
        mongoTemplate
                .find(searchQuery, TaskInProgressDocument.class)
    ...
    private void removeWorkflowDefToWorkflowMapping(Workflow workflow) {
        ...
        Query deleteQuery = new Query();
        deleteQuery.addCriteria(
                Criteria.where("workflow_def")
                        .is(workflow.getWorkflowName())
                        .and("date_str")
                        .is(dateStr(workflow.getCreateTime()))
                        .and("workflow_id")
    ...
    private Workflow readWorkflow(String workflowId) {
        ...
        return null != workflowDocument
                ? readValue(workflowDocument.getJson_data(), Workflow.class)
                : null;
    }
    ...
    public List<EventExecution> getEventExecutions(
        ...
        try {
            ...
            for (int i = 0; i < max; i++) {
                String executionId =
                        messageId + "_"
                                + i; // see SimpleEventProcessor.handle to understand how the
                // execution id is set
                EventExecution ee =
    ...
    private EventExecution readEventExecution(
            String eventHandlerName, String eventName, String messageId, String executionId) {
    ...
    private void insertOrUpdatePollData(PollData pollData, String domain) {
        ...
        long rowsUpdated =
                mongoTemplate
                        .updateMulti(updateQuery, update, PollDataDocument.class)
                        .getModifiedCount();
    ...
    private PollData readPollData(String queueName, String domain) {
        ...
        Query searchQuery = new Query();
        searchQuery.addCriteria(
                Criteria.where("queue_name").is(queueName).and("domain").is(domain));

        PollDataDocument pollDataDocument =
                mongoTemplate.findOne(searchQuery, PollDataDocument.class);

        return null != pollDataDocument
                ? readValue(pollDataDocument.getJson_data(), PollData.class)
                : null;
    }
    ...
    private List<PollData> readAllPollData(String queueName) {
        ...
        pollDataDocuments.forEach(
                pdd -> pollDataList.add(readValue(pdd.getJson_data(), PollData.class)));
