...
public class MongoMetadataDAO extends MongoBaseDAO implements MetadataDAO, EventHandlerDAO {
    ...
    public Optional<WorkflowDef> getLatestWorkflowDef(String name) {
        try {
            ...
            return Optional.ofNullable(metaWorkflowDefDocument)
                    .map(
                            row ->
                                    readValue(
                                            metaWorkflowDefDocument.getJson_data(),
                                            WorkflowDef.class));
        } catch (Exception e) {
        ...
        try {
            ...
            if (deletedCount == 0) {
                ...
                throw new ApplicationException(
                        ...
                        String.format(
                                "No such workflow definition: %s version: %d", name, version));
            } else {
    ...
    public void updateEventHandler(EventHandler eventHandler) {
        ...
        if (!mongoTemplate.exists(searchQuery, MetaEventHandlerDocument.class)) {
            throw new ApplicationException(
                    ApplicationException.Code.CONFLICT,
                    "EventHandler with name " + eventHandler.getName() + " doesn't exists!");
        }
    ...
    public List<EventHandler> getAllEventHandlers() {
        ...
        try {
            ...
            return metaEventHandlerDocuments.stream()
                    .map(
                            metaEventHandlerDocument ->
                                    readValue(
                                            metaEventHandlerDocument.getJson_data(),
                                            EventHandler.class))
                    .collect(Collectors.toList());
        } catch (Exception e) {
    ...
    public List<EventHandler> getEventHandlersForEvent(String event, boolean activeOnly) {
        ...
        try {
            List<MetaEventHandlerDocument> metaEventHandlerDocuments =
                    !activeOnly
                            ? mongoTemplate.find(
                                    new Query().addCriteria(Criteria.where("event").is(event)),
                                    MetaEventHandlerDocument.class)
                            : mongoTemplate.find(
                                    new Query()
                                            .addCriteria(
                                                    Criteria.where("event")
                                                            .is(event)
                                                            .and("active")
                                                            .is(activeOnly)),
                                    MetaEventHandlerDocument.class);
    ...
    private TaskDef getTaskDefFromDB(String name) {
        try {
            ...
            return !mongoTemplate.exists(searchQuery, MetaTaskDefDocument.class)
                    ? null
                    : readValue(
                            mongoTemplate
                                    .findOne(searchQuery, MetaTaskDefDocument.class)
                                    .getJson_data(),
                            TaskDef.class);
        } catch (Exception e) {
    ...
    private List<TaskDef> getAllTaskDefsFromDB() {
        ...
        try {
            ...
            return metaTaskDefDocuments.stream()
                    .map(
                            metaTaskDefDocument ->
                                    readValue(metaTaskDefDocument.getJson_data(), TaskDef.class))
                    .collect(Collectors.toList());
        } catch (Exception e) {
    ...
    private EventHandler getEventHandler(String name) {
        ...
        Query searchQuery = new Query();
        searchQuery.addCriteria(Criteria.where("name").is(name));
        return mongoTemplate.exists(searchQuery, MetaEventHandlerDocument.class)
                ? readValue(
                        mongoTemplate
                                .findOne(searchQuery, MetaEventHandlerDocument.class)
                                .getJson_data(),
                        EventHandler.class)
                : null;
    }
        ...
        MetaWorkflowDefDocument aMetaWorkflowDefDocument =
                mongoTemplate.findOne(
                        new Query()
                                ...
                                .with(Sort.by(Direction.DESC, "version")),
                        MetaWorkflowDefDocument.class);
        return Optional.ofNullable(
    ...
    public List<WorkflowDef> getAllLatest() {
        ...
        mongoTemplate
                .find(query, MetaWorkflowDefDocument.class)
                .forEach(
                        wdd -> {
