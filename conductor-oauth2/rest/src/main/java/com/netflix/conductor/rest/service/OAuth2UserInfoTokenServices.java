...
@ConditionalOnProperty(
        name = "conductor.api.security.enabled",
        value = "true",
...
public class OAuth2UserInfoTokenServices implements ResourceServerTokenServices {
        ...
        if (pathKeys.length == 1) {
            if (jsonElement.isJsonArray()) {
                ...
                if (jsonElement.getAsJsonObject().get(pathKeys[0]).isJsonArray()) {
                    for (JsonElement aSubElement :
                            jsonElement.getAsJsonObject().get(pathKeys[0]).getAsJsonArray()) {
                    ...
                    rolesList.addAll(
                            Arrays.asList(
                                    jsonElement
                                            ...
                                            .get(pathKeys[0])
                                            .getAsString()
                                            .split(ConductorRestConstants.COMMA)));
                }
            }
        } else {
            if (jsonElement.isJsonArray()) {
                ...
                rolesList =
                        extractRoles(
                                jsonElement.getAsJsonObject().get(pathKeys[0]),
                                Arrays.copyOfRange(pathKeys, 1, pathKeys.length));
            }
        }
    ...
    private OAuth2Authentication extractAuthentication(Map<String, Object> map) {
        ...
        try {
            ...
            if (this.userInfoUriParserPath != null) {
                String[] pathKeys =
                        this.userInfoUriParserPath.split(ConductorRestConstants.FORWARD_SLASH);

                if (null != map.get(pathKeys[0]) && pathKeys.length > 0) {
                    JsonElement rootElement =
                            new JsonParser().parse(gson.toJson(map.get(pathKeys[0])));
    ...
    /**
     * Get map is a custom implementation and extracts the OAuth2 Access information for the JWS
     * token returned by ADSF
     *
     * @param path
     * @param accessToken
     * @return
    ...
    private Map<String, Object> getMap(String path, String accessToken) {
        ...
        try {
            ...
            this.logger.warn(
                    "Could not fetch user details: " + ex.getClass() + ", " + ex.getMessage());
