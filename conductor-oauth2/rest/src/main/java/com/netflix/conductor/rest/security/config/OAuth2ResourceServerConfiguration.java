...
public class OAuth2ResourceServerConfiguration extends ResourceServerConfigurerAdapter {
    ...
    public void configure(final HttpSecurity http) throws Exception {
        ...
        if (null != this.resourceMapping) {
            ...
            ResourceRoleMappingDTO[] resRoleMappingDTOs =
                    gson.fromJson(this.resourceMapping, ResourceRoleMappingDTO[].class);
            ...
            if (null != resRoleMappingDTOs && resRoleMappingDTOs.length > 0) {
                ...
                finalLog =
                        finalLog
                                + "\nResource Mapping to be done for : "
                                + resRoleMappingDTOs.length
                                + " path mappings\n";
                ...
                for (ResourceRoleMappingDTO aResourceRoleMapping : resRoleMappingDTOs) {
                    ...
                    finalLog =
                            finalLog
                                    ...
                                    + pathCounter
                                    + ")\t Starting to Map: \t"
                                    + aResourceRoleMapping.getEndpoint();

                    String allowedRoles = "";

                    for (String aRole : aResourceRoleMapping.getRoles()) {
                        if (ConductorRestConstants.STRING_INITIALIZR.equalsIgnoreCase(
                                allowedRoles)) {
                            allowedRoles = aRole;
                        } else {
                        ...
                        switch (aHTTPMethod) {
                            case ConductorRestConstants.GET:
                                http.authorizeRequests()
                                        .antMatchers(
                                                HttpMethod.GET, aResourceRoleMapping.getEndpoint())
                                ...
                                finalLog =
                                        finalLog
                                                ...
                                                + "\n\t Mapped Roles: "
                                                + Arrays.asList(aResourceRoleMapping.getRoles())
                                                        .toString()
                            ...
                            case ConductorRestConstants.POST:
                                http.authorizeRequests()
                                        .antMatchers(
                                                HttpMethod.POST, aResourceRoleMapping.getEndpoint())
                                        .hasAnyAuthority(allowedRoles);
                                finalLog =
                                        finalLog
                                                + "\n\n############################## OAUTH2 API Security Mapping - START ####################################"
                            ...
                            case ConductorRestConstants.PUT:
                                http.authorizeRequests()
                                        .antMatchers(
                                                HttpMethod.PUT, aResourceRoleMapping.getEndpoint())
                                        .hasAnyAuthority(allowedRoles);
                                finalLog =
                                        finalLog
                                                + "\n\n############################## OAUTH2 API Security Mapping - START ####################################"
                            ...
                            default:
                                logger.error(
                                        "No matching HTTP Method case found for - " + aHTTPMethod);
                                break;
                        }
                    }
        ...
        http.authorizeRequests()
                .antMatchers(HttpMethod.GET, ConductorRestConstants.GENERIC_ROOT_URL)
                .permitAll()
                .anyRequest()
                .authenticated()
                .and()
