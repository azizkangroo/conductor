...
import com.netflix.conductor.oracle.util.ClobHandler;
...
public class OracleMetadataDAO extends OracleBaseDAO implements MetadataDAO, EventHandlerDAO {
    ...
    private static final String CLASS_NAME = OracleMetadataDAO.class.getSimpleName();

    public OracleMetadataDAO(
            ObjectMapper objectMapper, DataSource dataSource, OracleProperties properties) {
    ...
    public void addEventHandler(EventHandler eventHandler) {
        ...
        try {
            ...
            logger.error("Error while inserting Event Handler JSON DATA - {}", e.getMessage());
        }
    }
    ...
    public void updateEventHandler(EventHandler eventHandler) {
        ...
        try {
            ...
            logger.error("Error while updating Event Handler JSON DATA - {}", e.getMessage());
        }
    }
                            ...
                            rs -> {
                                ...
                                while (rs.next()) {
                                    try {
                                        ...
                                        EventHandler h =
                                                readValue(rs.getString(1), EventHandler.class);
                                        if (!activeOnly || h.isActive()) {
                                            handlers.add(h);
                                        }
                                    }
    ...
    private Optional<Integer> getLatestVersion(Connection tx, String name) {
        final String GET_LATEST_WORKFLOW_DEF_VERSION =
                "SELECT max(version) FROM meta_workflow_def WHERE " + "name = ?";
    ...
    private void insertOrUpdateWorkflowDef(Connection tx, WorkflowDef def) {
        ...
        try {
            ...
            logger.error(
                    "Error while inserting or updating Workflow Def JSON DATA - {}",
                    e.getMessage());
        }
    }
        ...
        return getWithRetriedTransactions(
                tx -> {
                    try {
                        ...
                        ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
                        String json = ow.writeValueAsString(taskDef);
                        ...
                        logger.error(
                                "Error while inserting or updating Task Def JSON DATA - {} - JSON {}",
                                e.getMessage(),
                                taskDef.toString());
                        e.printStackTrace();
                        return null;
